...
## Phase 7C – Graphite Metrics

Graphite (started in Phase 7A) now actually receives and stores metrics, generated by a small script rather than a third-party exporter — no Prometheus, Telegraf, collectd, or StatsD client was introduced.

**How metrics are generated:** `monitoring/graphite/scripts/publish-metrics.sh` runs real `curl` checks (the same targets Nagios monitors — Jenkins, the TechnoVista website via its dynamically-discovered Kubernetes NodePort, and the Kubernetes API) and computes, per target: availability (`1`/`0`), response time in milliseconds, and the raw HTTP status code. Nothing is fabricated — every value comes from an actual request made at send time.

**How metrics reach Graphite:** each measurement is sent as `devops.<target>.<metric> <value> <unix-timestamp>` over a plain TCP socket to Carbon's plaintext receiver on port 2003 (bash's `/dev/tcp`, no `nc` dependency). `monitoring/graphite/scripts/run-loop.sh [interval]` wraps the script in a loop (default 60s) for continuous publishing.

**How to verify ingestion:** query Graphite's render API directly —
```
